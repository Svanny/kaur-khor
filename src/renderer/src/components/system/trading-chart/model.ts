  ...
  const resolvedPoints = [...buckets.entries()]
    ...
    .map(([bucketTime, members]) => {
      ...
      return {
        ...
        demandMinusAvailableCapacity: (() => {
          const available = lastDefined(sortedMembers.map(p => p.availableCapacity));
          if (available == null) return null;
          const service = lastDefined(sortedMembers.map(p => p.serviceDemandMean)) ?? 0;
          const retail = lastDefined(sortedMembers.map(p => p.retailDemandMean)) ?? 0;
          const demand = (service ?? 0) + (retail ?? 0);
