...
import { buildCaptureSessionHref, type CaptureSessionAction, type CaptureSessionTargetType } from '@/lib/navigation/record-update-routes';
import {
  ...
  return visibleCatalog.skus.flatMap((sku) => {
    ...
    const commands: CommandDescriptor[] = [
      createCommand({
        ...
        pageId: 'catalog',
        pageOrder: 3,
        pagePrefixes: ['/catalog', buildSkuDetailHref(sku.skuId)],
        priority: 200,
        subtitle: skuSubtitle,
        title: sku.name,
      }),
      createCommand({
        action: { href: buildSkuEditHref(sku.skuId), type: 'workflow' },
    ...
    return [
      ...
      ...sheetCommands.map((command, index) =>
        createCommand({
          ...
          pageId: 'catalog',
          pageOrder: 3,
          pagePrefixes: [buildSkuDetailHref(sku.skuId)],
          priority: 220 + index,
  ...
  return visibleCatalog.services.flatMap((service) => {
    ...
    return [
      createCommand({
        ...
        pageId: 'catalog',
        pageOrder: 3,
        pagePrefixes: ['/catalog', buildServiceDetailHref(service.serviceId)],
        priority: 230,
        subtitle: `Service · ${service.serviceId}`,
        title: service.name,
      }),
      createCommand({
        action: { href: buildServiceEditHref(service.serviceId), type: 'workflow' },
      ...
      ...sheetCommands.map((command, index) =>
        createCommand({
          ...
          pageId: 'catalog',
          pageOrder: 3,
          pagePrefixes: [buildServiceDetailHref(service.serviceId)],
          priority: 250 + index,
  ...
  for (const task of model.tasks) {
    ...
    commands.push(
      createCommand({
        ...
        pageId: 'work',
        pageOrder: 0,
        pagePrefixes: ['/work', buildSkuDetailHref(task.skuId)],
        priority: 310,
