...
For phone portrait browser views, render the embedded product shell through the landscape-first wrapper instead of depending on `screen.orientation.lock()`. The wrapper owns the rotated scroll area, viewport CSS variables, and embedded shell attributes; route components should use those shared markers rather than detecting phone landscape locally. Normal component-level responsive swaps, container queries, and measured controls still belong inside their local components; do not duplicate app-wide threshold math in route code.
