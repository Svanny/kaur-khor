...
For phone portrait browser views, block the embedded product shell with the rotate-screen overlay instead of depending on `screen.orientation.lock()`. The normal browser app and demo should disable the user-facing phone workaround and render actual phone landscape viewports through the larger embedded layout. Keep the phone-landscape wrapper available as an opt-in code path only; when enabled, the wrapper owns the rotated scroll area, viewport CSS variables, and embedded shell attributes. Normal component-level responsive swaps, container queries, and measured controls still belong inside their local components; do not duplicate app-wide threshold math in route code.
