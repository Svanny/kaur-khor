...
For phone portrait browser views, render the dedicated phone operator shell instead of rotating or scaling the desktop shell. The normal browser app and demo should keep the portrait viewport upright, expose phone-specific routes for Today, Queue, Capture, Products, and More, and reserve the larger embedded desktop layout for wider or landscape viewports. Keep the phone-landscape wrapper available as an opt-in code path only; when enabled, the wrapper owns the rotated scroll area, viewport CSS variables, and embedded shell attributes. Normal component-level responsive swaps, container queries, and measured controls still belong inside their local components; do not duplicate app-wide threshold math in route code.
