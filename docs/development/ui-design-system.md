...
For public phone portrait browser views, show the operator shell instead of exposing the desktop shell in a cramped upright frame. The normal browser app and demo should use the phone shell for Today, Queue, Capture, Products, Insights, and workspace safety actions. Phone Products should stay route-native for lookup and summary detail instead of falling through to desktop detail routes. Phone Capture should keep the hub as a compact action menu, then place deeper capture sessions inside the horizontally scrollable capture surface. Landscape phone and wider views keep the full embedded workspace. Normal component-level responsive swaps, container queries, and measured controls still belong inside their local components; do not duplicate app-wide threshold math in route code.
