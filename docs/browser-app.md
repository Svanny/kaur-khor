...
- Phone portrait views keep the rotated landscape product shell and automatic zoom policy active behind an upright solid warning that asks operators to turn the screen sideways. The warning copy and disabled Done button cycle between English and Khmer with the onboarding text animation timing while the underlying landscape shell remains ready for zoom-in and zoom-out when the viewport changes.
