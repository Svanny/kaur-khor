...
- API image builds in GitHub Actions use Docker buildx with `apps/api` as the build context and persistent `type=gha` cache scopes for release and PR validation paths.
- Docker dependency caching uses `cargo-chef` inside [`apps/api/Dockerfile`](/Users/svanny/banji/apps/api/Dockerfile).
