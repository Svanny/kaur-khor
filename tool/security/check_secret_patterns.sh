  ...
  while IFS= read -r file; do
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
      ...
      if [[ "$line" =~ ^[[:space:]]*([A-Z0-9_]+)[[:space:]]*=(.*)$ ]]; then
        ...
        if [[ "$key" =~ ^(DATABASE_RUNTIME_URL|DATABASE_MIGRATION_URL|REDIS_URL|RABBIT_URL|OBJECT_STORAGE_ACCESS_KEY|OBJECT_STORAGE_SECRET_KEY|OTEL_HEADERS|.*(SECRET|TOKEN|PASSWORD|API_KEY|ACCESS_KEY))$ ]]; then
