...
add_service() {
  ...
  if [[ -z "$service_id" ]]; then
    echo "error: $env_name is required for prod Railway shutdown" >&2
    exit 1
  fi
...
main() {
  ...
  if [[ "$RAILWAY_ENVIRONMENT" != "prod" ]]; then
    echo "error: RAILWAY_ENVIRONMENT must be 'prod' for the prod shutdown script" >&2
    exit 1
  fi
  ...
  add_service "api" "RAILWAY_PROD_API_SERVICE_ID"
  add_service "event-relay" "RAILWAY_PROD_EVENT_RELAY_SERVICE_ID"
  add_service "projection-consumer" "RAILWAY_PROD_PROJECTION_CONSUMER_SERVICE_ID"
  add_service "worker" "RAILWAY_PROD_WORKER_SERVICE_ID"
  add_service "frontend" "RAILWAY_PROD_FRONTEND_SERVICE_ID"
  add_service "keycloak" "RAILWAY_PROD_KEYCLOAK_SERVICE_ID"
  add_service "db-ops" "RAILWAY_PROD_DB_OPS_SERVICE_ID"
