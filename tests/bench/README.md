...
```bash
node ./tools/scripts/run-benchmarks.mjs --list-managed-scenarios
```
...
`config/test/playwright.bench.config.ts`, seed the selected fixture before launch, and write
scenario summaries from the same persisted event stream. Keep new benchmark
options wired through both `tools/scripts/run-benchmarks.mjs`,
`src/main/benchmark/benchmark-runner.ts`, and the shared scenario metadata before changing
scenario behavior.
