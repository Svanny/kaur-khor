...
node tools/scripts/stage-desktop-core.mjs --platform=darwin --arch="${target_arch}"
pnpm build
pnpm exec electron-builder install-app-deps --platform=darwin --arch="${target_arch}"
pnpm exec electron-builder --mac dmg --config config/package/electron-builder.yml --publish never --"${target_arch}"
install_app_from_release
