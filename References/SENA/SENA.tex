...
\begin{abstract}
SENA (Sparse-stock Estimation with Nonstationary Adaptation) is a Bayesian generative model for settings with a single business entity where services (and optionally retail SKUs) are sold but inventory is recorded intermittently. From sparse stock snapshots, SENA infers a plausible latent ledger of service demand, retail demand, order placements, receipts, and adjustments, while adapting to drift, seasonality, promotions, and occasional change points.
...
\begin{enumerate}
  \item A Bayesian state-space model for a single business entity that reconstructs latent demand, order placements, receipts, and corrections from sparse stock snapshots.
  \item A practical identifiability strategy that can absorb user-supplied optional signals (rankings, order placed/received logs, sharing masks, stockout flags, observed price series, and noisy lead-time inputs).
  \item A planning layer that maps posterior latent demand and latent lead-time dynamics into stockout-risk, days-of-cover, and reorder-policy outputs.
...
\textbf{Content.} The next sections define the generative model, optional observation channels, prior defaults, and posterior outputs; later sections outline evaluation expectations and practical inference workflows.
...
\begin{enumerate}
  ...
  \[
    x_{1,i}=x^{\mathrm{init}}_i,\qquad B_{1,i}=B^{\mathrm{init}}_i,\qquad x^{\mathrm{init}}_i\ge 0,\ B^{\mathrm{init}}_i\ge 0.
  \]
  A practical default when pre-$t_1$ order history is unavailable is $B^{\mathrm{init}}_i=0$.
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item $\tilde c^L_{k,i}\in\{1,\dots,C\}$ (optional): user-selected variability class derived from the range (e.g., very stable, moderate, volatile)
  \end{enumerate}
...
For each SKU $i$, with initial states $(x_{1,i},B_{1,i})$ and transitions over $k=2,\dots,K$:
...
In this formulation, explicit bundles are expected to improve identifiability and interpretability; Section~\ref{sec:eval-protocol} specifies ablations to test this claim.
...
\]
where $\epsilon_L>0$ is a fixed lead-time floor in day units (default $\epsilon_L=0.5$ day).
...
\[
  \tilde c^L_{k,i}\sim\text{OrderedLogit}\!\bigl(a_{0,i}+a_{1,i}\psi_{k^{c,\star}_{k,i},i},\ \kappa_{1:C-1}\bigr),\quad a_{1,i}>0.
\]

Thus, users do not need to provide variance directly; range entries become an interpretable ordinal signal for latent lead-time variance.
...
Here $s^{\mathrm{adj}}_{r,i}>0$ is the unit-time correction noise scale (in SKU units) for SKU $i$ in regime $r$.
...
$O_{k,i}\mid(G_{k,i}=1)\sim\text{LogNormal}(\mu^O_{k,i},\sigma^O_i)$ and controls multiplicative variability in placed order sizes.

Default: initialize $\sigma^{O}_i=0.3$ (on the log scale) after scaling; smaller values imply more consistent order sizes.
...
\begin{enumerate}
  ...
  using $k^{c,\star}_{k,i}=\max\{1,k-\delta^c_{k,i}\}$ in the lagged state index.
...
\begin{enumerate}
  ...
  \]
  SENA outputs posterior summaries for $m^L_{k,i}$ (mean, days) and $v^L_{k,i}$ (variance, days$^2$), and for induced lead-time draws under the same LogNormal default (using Eqs.~\eqref{eq:leadtime-map} and~\eqref{eq:leadtime-draw}):
  \[
  ...
  \end{equation}
  Equations~\eqref{eq:lt-demand-approx}--\eqref{eq:rop-trigger} summarize the default planning definitions used for safety stock and reorder decisions.
...
\begin{enumerate}
  ...
  \item \textbf{Within-interval timing is collapsed.} SENA models net effects over each reporting interval $\Delta t_k$. If placements, receipts, and consumption interleave within an interval, exact stockout timing and lost-sales duration can be weakly identified without finer-grained event data.
